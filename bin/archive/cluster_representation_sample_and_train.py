...
def main(argv):
    ...
    parser.add_argument('--frac', action='store', type=float, default=None)
    parser.add_argument('--distribution', action='store', type=str, choices=['uniform', 'frequency', 'inverse_frequency'], default='uniform')
...
#### CLUSTER AND CREATE NEW DATASET #####
    ...
    if frac is not None:
      ...
      new_data = stratified_sampling(data, new_data, frac=frac, distribution=distribution)

    percentage = new_data.shape[0]/(new_data.shape[0] + len(data.train_triples))*100
...
#### TRAIN COMPLEX #####
    ...
    for metrics, name in [[best_metrics_dev, 'dev'], [best_metrics_test, 'test']]:
        results_original = pd.DataFrame({'dataset': data_name, 'dev/test': name, 'n_clusters' : n_clusters , 'cluster_type': cluster_type, 'metric': metric, 'embeddings_string' : embeddings_path, 'frac':frac, 'distribution':distribution, 'percentage':percentage,  'MRR': metrics['MRR'] , 'H1': metrics['hits@1'], 'H3':  metrics['hits@3'], 'H5': metrics['hits@5'], 'H10': metrics['hits@10'], 'H50': metrics['hits@50'], 'H100': metrics['hits@100'], 'best_epoch': best_epoch,'lr': learning_rate, 'F2': F2_weight, 'N3': N3_weight, 'seed' : seed, 'model':model_name,  'embedding_size': embedding_size, 'batch_size': batch_size, 'nb_epochs':nb_epochs}, index=[0])
